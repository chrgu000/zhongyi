<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"E:\wamp\www\transport\public/../application/admin\view\ordergoodsku\order-list.html";i:1506419140;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<![endif]-->
<title>找库货源订单管理</title>
</head>
<body>
<nav class="breadcrumb">
    <i class="Hui-iconfont">&#xe67f;</i> 首页 
    <span class="c-gray en">&gt;</span> 订单管理 
    <span class="c-gray en">&gt;</span> 找库货源 
    <!--<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" 
    href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a>-->
</nav>
<div class="pd-20 nullwrap">
<div class="text-c">
<form action="<?php echo url('Ordergoodsku/lst'); ?>" method="get">
    订单状态：
...
<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top: 25px;">
    <span class="l">
        <a class="btn btn-danger radius" href="<?php echo url('Ordergoodsku/export',array('order_state'=>session('findwhorder_state'),'order_number'=>session('goodswhorder_number'),'time'=>session('goodswhordertime'))); ?>" style="margin-right: 5px;">
	    <i class="Hui-iconfont">&#xe632;</i> 导出数据</a>
    </span>
	<form action="<?php echo url('Ordergoodsku/p_del'); ?>" method="post"> 
	<?php if(in_array('141',$privilege)){ ?>
        <span class="l">
		   ...
		   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<p class='nulltext'>暂无相关信息</p>" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				...
				<td >
				    ...
				    <?php elseif($v['pay_state'] == 3): ?>已入库
				    <?php elseif($v['pay_state'] == 4): ?>已取消
				    <?php elseif($v['pay_state'] == 5): ?>退款中
				    <?php endif; ?>
				</td>
				<td >
				    <?php if($v['ku_state'] == 1): ?>待付款
				    <?php elseif($v['ku_state'] == 2): ?>待入库
				    <?php elseif($v['ku_state'] == 3): ?>已入库
				    <?php elseif($v['ku_state'] == 4): ?>已取消				        
				    <?php endif; ?>
				</td>
				<td >
				    <?php if($v['order_state'] == 1): ?>待付款
				    <?php elseif($v['order_state'] == 2): ?>待入库
				    <?php elseif($v['order_state'] == 3): ?>已完成
				    <?php elseif($v['order_state'] == 4): ?>已取消
				    <?php elseif($v['order_state'] == 5): ?>退款中
				    <?php endif; ?>
				</td>
				<td><?php echo $v['time']; ?></td>
				<td  class="td-manage">
				<?php if(in_array('142',$privilege)){ if($v['order_state'] == 5): ?>
				    <a title="退款" href="<?php echo url('Ordergoodsku/back',array('id'=>$v['id'])); ?>" onclick="return confirm('确定同意退款？')"  class="ml-5" style="color: #333; text-decoration: none;">
                    确认退款&nbsp;</a><br/>
                <?php endif; } if(in_array('143',$privilege)){ if($v['order_state'] == 2): ?>
				    <a title="完成" href="<?php echo url('Ordergoodsku/finish',array('id'=>$v['id'])); ?>" onclick="return confirm('确定完成订单？')"  class="ml-5" style="color: #333; text-decoration: none;">
                    确认支付</a><br/>
                <?php endif; } ?>
                    <a title="查看" href="<?php echo url('Ordergoodsku/detail',array('id'=>$v['id'])); ?>"  class="ml-5" style="color: #333; text-decoration: none;">
                    详情&nbsp;|</a> 
                <?php if(in_array('140',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('Ordergoodsku/del',array('id'=>$v['id'])); ?>"  onclick="return confirm('确定要删除？')" class="ml-5" style="color: #333; text-decoration: none;">
                    删除</a><?php } ?>
                </td>
			</tr>
