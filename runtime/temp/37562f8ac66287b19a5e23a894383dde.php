<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:81:"E:\WWW\transport\public/../application/index\view\warehouse\warehouse_detail.html";i:1508720814;s:68:"E:\WWW\transport\public/../application/index\view\common\header.html";i:1508719130;s:68:"E:\WWW\transport\public/../application/index\view\common\footer.html";i:1507716955;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8">
...
<div class="contanier">
  <div class="wrap-1200">
    <a class="back" href="<?php echo url('Warehouse/warehouse'); ?>">返回</a>
    <!-- <button class="back" class="btn btn-default btn-sm btn-right" onclick="history.go(-1)">返回</button> -->
    <?php if(session('userid') != ''): if($user[0]['check_state'] == '1'): if($warehouse['order_state'] == '1'): if(session('username')  ==  $warehouse['wh_uid']): ?>
            <a class="back" style="margin-right: 20px;background-color: gray;color: black;" >您的库源</a>
          <?php else: ?>
            <form method="get" action="<?php echo url('Warehouse/warehouseorder'); ?>" target="frame" >
              ...
              <?php if(empty($order_warehouse) != true): if(session('userid') == $order_warehouse[0]['user_id']): if(in_array($warehouse['id'],$whids)): ?>
                    <a class="back" style="margin-right: 20px;background-color: gray;color: black;" >您已抢单</a>
                  <?php else: ?>
                    <a class="back" id="a" href="<?php echo url('Order/order_warehouse',array('id'=>$warehouse['id'])); ?>" data-toggle="modal" data-target="#<?php echo $warehouse['id']; ?>" style="margin-right: 20px;">选择仓库</a>
                  <?php endif; else: ?>
                  <a class="back" id="a" href="<?php echo url('Order/order_warehouse',array('id'=>$warehouse['id'])); ?>" data-toggle="modal" data-target="#<?php echo $warehouse['id']; ?>" style="margin-right: 20px;">选择仓库</a>
                <?php endif; else: ?>
                <a class="back" id="a" href="<?php echo url('Order/order_warehouse',array('id'=>$warehouse['id'])); ?>" data-toggle="modal" data-target="#<?php echo $warehouse['id']; ?>" style="margin-right: 20px;">选择仓库</a>
          ...
          <?php endif; elseif($warehouse['order_state'] == '2'): ?>
          <a class="back" style="margin-right: 20px;background-color: gray;color: black;" >被抢单</a>
        <?php else: ?>
          <a class="back" style="margin-right: 20px;background-color: gray;color: black;" >已完成</a>
        <?php endif; else: ?>
    ...
    <?php endif; ?>
    <div class="modal fade" id="<?php echo $warehouse['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="md-content">
          <h3>下单</h3>
          <div>
          <iframe name="frame" ID="Frame1" SRC="<?php echo url('Warehouse/warehouseorder'); ?>"  style="width:520px; border:none; min-height: 600px; "> </iframe>
          </div>
        </div>
      </div>
    </div>
