<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:85:"E:\wamp\www\transport\public/../application/admin\view\ordergoodsku\recycle-list.html";i:1506419144;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="tabBar cl">
    <a href="<?php echo url('Ordergoodsku/recyclelst'); ?>" style="color: #FFF; text-decoration: none;"><span>找库货源</span></a>
...
<div class="pd-20">
<div class="text-c">
<form action="<?php echo url('Ordergoodsku/recyclelst'); ?>" method="get">
    订单状态：
...
<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top: 25px;">
    <span class="l">
        <a class="btn btn-danger radius" href="<?php echo url('Ordergoodsku/backexport',array('order_state'=>session('findwhrecycleorder_state'),'order_number'=>session('goodswhrecycleorder_number'),'time'=>session('goodswhrecycleordertime'))); ?>" style="margin-right: 5px;">
	    <i class="Hui-iconfont">&#xe632;</i> 导出数据</a>
    </span>
	<form action="<?php echo url('Ordergoodsku/recycle_p_del'); ?>" method="post"> 
		...
		<tbody>
		   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<p class='nulltext'>暂无相关信息</p>" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				...
				<td  class="td-manage">                 
                    <!--<a title="查看" href="<?php echo url('Ordergoodsku/detail',array('id'=>$v['id'])); ?>"  class="ml-5" style="text-decoration:none" >
                    <i class="Hui-iconfont">&#xe665;</i>&nbsp;&nbsp;</a> -->
                    <?php if(in_array('148',$privilege)){ ?>
                    <a href="<?php echo url('Ordergoodsku/recycle_back',array('id'=>$v['id'])); ?>" onclick="return confirm('确定要还原吗？')" title="还原" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe66b;</i>&nbsp;&nbsp;</a><?php } if(in_array('149',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('Ordergoodsku/recycle_del',array('id'=>$v['id'])); ?>" onclick="return confirm('确定要删除吗？')" class="ml-5" style="text-decoration:none">
