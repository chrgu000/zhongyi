<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:74:"E:\wamp\www\transport\public/../application/admin\view\user\user-list.html";i:1507622046;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="pd-20 nullwrap">
	...
	<form action="<?php echo url('User/lst'); ?>" method="get">
	  <span class="select-box inline">
	    <select name="userkeytype" class="select" style="width: 130px;">
        ...
        账号：<input type="text" class="input-text" style="width:150px" placeholder="请输入账号" id="" name="keywords"
        value="">
        用户id：<input type="text" class="input-text" style="width:150px" placeholder="请输入账号" id="" name="keyid"
        value="">
	...
	<span class="l">
        <a class="btn btn-danger radius" href="<?php echo url('User/userexport',array('keytype'=>session('userkeytype'),'keywords'=>session('userkeywords'),'keyid'=>session('userkeyid'))); ?>" style="margin-right: 5px;">
	        <i class="Hui-iconfont">&#xe632;</i> 导出数据
	    </a>
    </span>
	<form action="<?php echo url('User/p_del'); ?>" method="post"> 
	<?php if(in_array('77',$privilege)){ ?> 
	<span class="l">       
	...
	<table class="table table-border table-bordered table-bg nulltable">
		...
		<tbody>
			...
			<tr class="text-c">
                ...
                <?php if(in_array('75',$privilege)){ ?>
                    <a title="编辑"  href="<?php echo url('User/edit',array('id'=>$v['id'])); ?>" class="ml-5" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe6df;</i></a> <?php } if(in_array('76',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('User/del',array('id'=>$v['id'])); ?>"  onclick="return confirm('确定要删除吗？')" class="ml-5" style="text-decoration:none">
