<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:80:"E:\wamp\www\transport\public/../application/admin\view\article\article-list.html";i:1506418886;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="pd-20 nullwrap">
	...
	<form action="<?php echo url('Article/p_del'); ?>" method="post"> 
	<?php if(in_array('62',$privilege)){ ?>
        <span class="l">
		...
		<tbody>
		   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<p class='nulltext'>暂无相关信息</p>" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				...
				<td><?php echo $v['articletime']; ?></td>
				<td><?php echo date("Y-m-d",$v['createtime']); ?></td>
				<td class="td-manage">
				<?php if(in_array('60',$privilege)){ ?>
                    <a title="编辑"  href="<?php echo url('Article/edit',array('id'=>$v['id'])); ?>" class="ml-5" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe6df;</i></a> <?php } if(in_array('61',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('Article/del',array('id'=>$v['id'])); ?>"  onclick="return confirm('确定要删除吗？')" class="ml-5" style="text-decoration:none">
