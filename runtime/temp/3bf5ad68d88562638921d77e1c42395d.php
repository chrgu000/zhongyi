<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"E:\WWW\transport\public/../application/admin\view\entrustcarorder\recycle-list.html";i:1509441948;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="tabBar cl">
    <a href="<?php echo url('Entrustcarorder/recyclelst'); ?>" style="color: #FFF; text-decoration: none;"><span>货找车委托</span></a>
...
<div class="pd-20">
<div class="text-c">
<form action="<?php echo url('Entrustcarorder/recyclelst'); ?>" method="get">
    订单状态：
...
<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top: 25px;">
    <span class="l">
        <a class="btn btn-danger radius" href="<?php echo url('Entrustcarorder/backexport',array('order_state'=>session('goodsentrustcarrecycle_state'),'order_number'=>session('goodsentrustcarrecycle_number'),'time'=>session('goodsentrustcarrecycle_time'))); ?>" style="margin-right: 5px;">
	    <i class="Hui-iconfont">&#xe632;</i> 导出数据</a>
    </span>
	<form action="<?php echo url('Entrustcarorder/recycle_p_del'); ?>" method="post"> 
	<?php if(in_array('179',$privilege)){ ?>
        <span class="l">
            <button type="submit" class="btn btn-danger radius" onclick="return confirm('确定彻底删除吗？')" >
		   ...
		   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<p class='nulltext'>暂无相关信息</p>" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				...
				<td  class="td-manage">                 
                    ...
                    <?php if(in_array('177',$privilege)){ ?>
                    <a href="<?php echo url('Entrustcarorder/recycle_back',array('id'=>$v['id'])); ?>" onclick="return confirm('确定要还原吗？')" title="还原" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe66b;</i>&nbsp;&nbsp;</a><?php } if(in_array('178',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('Entrustcarorder/recycle_del',array('id'=>$v['id'])); ?>" onclick="return confirm('确定彻底删除吗？')" class="ml-5" style="text-decoration:none">
