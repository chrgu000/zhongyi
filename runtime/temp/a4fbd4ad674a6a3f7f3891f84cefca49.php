<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"E:\wamp\www\transport\public/../application/admin\view\ordergoodscar\order-list.html";i:1506419108;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<![endif]-->
<title>找车货源订单管理</title>
</head>
<body>
<nav class="breadcrumb">
    <i class="Hui-iconfont">&#xe67f;</i> 首页 
    <span class="c-gray en">&gt;</span> 订单管理 
    <span class="c-gray en">&gt;</span> 找车货源 
    <!--<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" 
    href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a>-->
</nav>
<div class="pd-20 nullwrap">
<div class="text-c">
<form action="<?php echo url('Ordergoodscar/lst'); ?>" method="get">
    订单状态：
...
<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top: 25px;">
    <span class="l">
        <a class="btn btn-danger radius" href="<?php echo url('Ordergoodscar/export',array('order_state'=>session('findcarorder_state'),'order_number'=>session('goodscarorder_number'),'time'=>session('goodscarordertime'))); ?>" style="margin-right: 5px;">
	    <i class="Hui-iconfont">&#xe632;</i> 导出数据</a>
    </span>
	<form action="<?php echo url('Ordergoodscar/p_del'); ?>" method="post"> 
	<?php if(in_array('130',$privilege)){ ?>
        <span class="l">
		...
		<tbody>
		   <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<p class='nulltext'>暂无相关信息</p>" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
			<tr class="text-c">
				...
				<td >
				    <?php if($v['pay_state'] == 1): ?>待支付
				    <?php elseif($v['pay_state'] == 2): ?>已支付
				    <?php elseif($v['pay_state'] == 3): ?>运输中
				    <?php elseif($v['pay_state'] == 4): ?>已完成
				    <?php elseif($v['pay_state'] == 5): ?>已取消
				    <?php elseif($v['pay_state'] == 6): ?>退款中
				    <?php endif; ?>
				</td>
				<td >
				    <?php if($v['car_state'] == 1): ?>待付款
				    <?php elseif($v['car_state'] == 2): ?>待发车
				    <?php elseif($v['car_state'] == 3): ?>已发车
				    <?php elseif($v['car_state'] == 4): ?>已完成
				    <?php elseif($v['car_state'] == 5): ?>已取消				        
				    <?php endif; ?>
				</td>
				<td >
				    <?php if($v['order_state'] == 1): ?>待付款
				    <?php elseif($v['order_state'] == 2): ?>待发车
				    <?php elseif($v['order_state'] == 3): ?>运输中
				    <?php elseif($v['order_state'] == 4): ?>已完成
				    <?php elseif($v['order_state'] == 5): ?>已取消
				    <?php elseif($v['order_state'] == 6): ?>退款中
				    <?php endif; ?>
				</td>
				<td><?php echo $v['time']; ?></td>
				<td  class="td-manage">
				<?php if(in_array('132',$privilege)){ if($v['order_state'] == 6): ?>
				    <a title="退款" href="<?php echo url('Ordergoodscar/back',array('id'=>$v['id'])); ?>" onclick="return confirm('确定同意退款？')"  class="ml-5" style="color: #333; text-decoration: none;">
                    确认退款&nbsp;</a><br/>
                <?php endif; } if(in_array('137',$privilege)){ if($v['order_state'] == 3): ?>
				    <a title="完成" href="<?php echo url('Ordergoodscar/finish',array('id'=>$v['id'])); ?>" onclick="return confirm('确定完成订单？')"  class="ml-5" style="color: #333; text-decoration: none;">
                    确认支付</a><br/>
                <?php endif; } ?>
                    <a title="查看" href="<?php echo url('Ordergoodscar/detail',array('id'=>$v['id'])); ?>"  class="ml-5" style="color: #333; text-decoration: none;">
                    详情&nbsp;|</a> 
                <?php if(in_array('129',$privilege)){ ?>
                    <a title="删除" href="<?php echo url('Ordergoodscar/del',array('id'=>$v['id'])); ?>"  onclick="return confirm('确定要删除？')" class="ml-5" style="color: #333; text-decoration: none;">
                    删除</a><?php } ?>
                </td>
			</tr>
